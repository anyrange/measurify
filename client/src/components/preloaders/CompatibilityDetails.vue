<template>
  <container>
    ...
    <container-item v-if="compatibilityData.genres.length">
      ...
      <horizontal-scroll>
        <badge v-for="(genre, index) in compatibilityData.genres" :key="index">
          {{ genre }}
        </badge>
      </horizontal-scroll>
    </container-item>
    <container-item v-if="compatibilityData.artists.length">
      <container-item-label> Artists Rank (Yours/Theirs) </container-item-label>
    ...
    <container-item v-if="compatibilityData.tracks.length">
      <container-item-label> Tracks Rank (Yours/Theirs) </container-item-label>
      <div class="flex gap-2 flex-wrap">
        <template
